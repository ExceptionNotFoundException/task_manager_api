...
Предположим, что в сущности `Task` есть поле `someInternalField`, которые не должно быть доступно для модификации и
...
В проекте для наглядности используется ручной маппинг, но на реальных проектах рекомендуется использовать готовую библиотеку.
...
При преобразовании объекта в JSON, его поле с `FetchType.LAZY` могут вызвать ошибку.
