...
2. В методе `setUp()`, выполняемом перед каждым тестом, инициализируются тестовые данные: создается 
...
задачи и проверяя HTTP статус ответа, структуру возвращаемого JSON и факт того что соответствующий метод сервиса был вызван.
